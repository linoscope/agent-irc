...
func (s *session) dispatch(m Message) {
	...
	case "QUIT":
		s.handleQuit(m)
	case "PING":
		// Chapter 03 implements proper keepalive; for now, just reply.
		if len(m.Params) > 0 {
...
func (s *session) maybeWelcome() {
	...
	nick := s.nick
	s.mu.Unlock()
	s.send(fmt.Sprintf("001 %s :Welcome to the chapter-02 IRC server, %s", nick, nick))
	s.send(fmt.Sprintf("002 %s :Your host is %s, running ch02", nick, serverName))
