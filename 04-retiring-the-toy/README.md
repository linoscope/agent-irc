...
# Then in weechat:
weechat
/server add ergo localhost/16670 -notls
/connect ergo
/join #room

# Or in irssi:
irssi
/connect localhost 16670
/join #room
```
...
4. **`caps/defs.go`** is generated by `python3 gencapdefs.py`. It enumerates every IRCv3 capability Ergo supports. We'll add an entry there in chapter 05b.
...
A useful first exercise: open `~/workspace/ergo/irc/handlers.go`, find `func messageHandler` (one handler shared by PRIVMSG, NOTICE, and TAGMSG — Ergo unifies them), and trace the broadcast path through `(*Channel).SendSplitMessage` in `irc/channel.go`. The shape will feel familiar — it's our chapter-02 fan-out, but with rate limiting, history persistence, message-splitting at the 512-byte boundary, and `account-tag` injection layered on.
...
[Chapter 05a — Capability negotiation](../05a-capability-negotiation) — understand IRCv3 CAP at the protocol level by probing unmodified Ergo with `/quote`. Then [Chapter 05b — Adding a vendor capability](../05b-vendor-capability) walks through the first fork modification: add `agent-irc.example/hello` and watch it appear on the wire. This is the warm-up before the real customization in chapter 07.
